...
rewrite_annotation() {
  local source="$1"
  local bound="$2"
  local tmp="$TMPDIR/$(basename "$source").rewrite"
...
rewrite_expected() {
  local tmp="$TMPDIR/expected.toml"
  {
...
time_check="$TMPDIR/time-check.txt"
if ! /usr/bin/time -v -o "$time_check" true >/dev/null 2>"$TMPDIR/time-check.err"; then
  echo "error: /usr/bin/time must support -v" >&2
  cat "$TMPDIR/time-check.err" >&2
  exit 1
fi
...
for source in "${PROGRAMS[@]}"; do
  ...
  binary="$TMPDIR/$name"
  build_stdout="$TMPDIR/$name.build.out"
  build_stderr="$TMPDIR/$name.build.err"
  run_stdout="$TMPDIR/$name.run.out"
  run_stderr="$TMPDIR/$name.run.err"
  time_log="$TMPDIR/$name.time.txt"
