...
| Module path           | Provides                                                                 | ESBMC status (2026-06-21) |
|-----------------------|-------------------------------------------------------------------------|---------------------------|
| `stdlib/math`         | `arithmetic`, `number_theory`, `vec_math` — integer & vector math        | Blocked (env)             |
| `stdlib/heap`         | `min_heap`, `max_heap` — binary heaps over `i64`                          | Partial / blocked         |
| `stdlib/stack`        | `stack` — Vec-backed LIFO stack over `i64`                               | Skipped (documentary)     |
| `stdlib/geometry`     | `point`, `shape` — 2D points, circles, rectangles                        | **Verified**              |
| `stdlib/bignum`       | `bignum` — arbitrary-precision signed integers                          | Skipped (documentary)     |
| `stdlib/gc`           | `gc` — mark-and-sweep garbage collector over `i64` slots                 | VerifyFailed (see notes)  |

"ESBMC status" is what `vow verify <module>/main.vow` reports today. It reflects
limitations of the verifier's memory model, **not** the soundness of the contracts.
