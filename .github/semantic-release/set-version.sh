...
# Discover workspace members exactly as bump-version.sh does, using only bash
# builtins (no sed/awk) since GNU sed's `addr1,addr2{ cmds }` block syntax
# isn't portable to macOS's BSD sed, and `mapfile` isn't available on macOS's
# bash 3.2 either.
TOMLS=()
in_workspace=0
