...
|------|---------------|-------|
| `O(1)` | 1.0 | Constant |
| `O(log(n))` | log(2n)/log(n) (~1.06–1.25 over n=16..65536) | Logarithmic |
| `O(n)` | 2.0 | Linear |
...
- Compare the observed ratio to a **class-specific *expected ratio function* r_class(n)**, not a single constant:
  ...
  | `O(log n)` | log(2n)/log(n) | ~1.25 at n=16, ~1.10 at n=1024, ~1.06 at n=65536. |
...
- **Maximum-class trend guard.** A class beyond the fixed set can still fit the largest candidate with a deceptively high R² (for example, `n⁴` or `n³ log² n` can fit `n³ log n` closely over a finite geometric grid). When `O(n³ log n)` is the best fit for an equally permissive declaration, compute the finite-difference slope of operation count against the `n³ log n` basis over each of the final three intervals. If those normalized slopes rise twice in succession under a relative tolerance, report `AMBIGUOUS` rather than `PASS`. Four samples are required to establish those three intervals; if the maximum candidate wins with only three samples, the trend evidence is insufficient and the verdict is also `AMBIGUOUS`. Comparing differences makes the guard insensitive to fixed setup costs in the operation count, while the relative tolerance makes it insensitive to positive rescaling of the counter unit; inspecting only the tail avoids letting small-input warmup noise determine the verdict. `AMBIGUOUS` is deliberately conservative: finite samples cannot prove whether a rising residual comes from a genuinely higher class or from lower-order threshold effects in a valid `O(n³ log n)` implementation. This guard does not make declarations with another log factor or polynomial degree 4 valid; the parse-time caps remain one log factor and polynomial degree 3.
...
- **Examples** against the fixed ordering above:
  - declared `O(n³)`, best fit `O(n)` (R²=0.99) → **PASS + tightness WARN** (loose but correct).
  - declared `O(n log n)`, best fit `O(n)` (R²=0.99) → **PASS + tightness WARN**.
...
PASS: complexity O(n * log(n)) confirmed
      Random inputs:  R^2 = 0.98, observed ratios in [2.20, 2.33]
                      (expected r_class(n) range over the size grid: [2.20, 2.50])
      Worst-case:     R^2 = 0.96, observed ratios in [2.20, 2.42]
...
avoids atomic counter traffic inside every sort comparison. A future
performance verdict must fail closed as unverified when it reaches a
size-dependent helper without a catalogued adapter; completing that broader
helper catalog is tracked by #486.
