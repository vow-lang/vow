...
- If `/simplify` genuinely cannot proceed (e.g. no open PR found for this
  branch), post a `gh pr comment` explaining what blocked you, then
...
found nothing to simplify), and no `BLOCKED.md` exists in the workspace.
The orchestrator will re-enter the wait state and start polling CI/merge
signals for this PR.
